...
const tooltip = $body.querySelectorAll('._tooltip');
if(tooltip) {
   for (let index = 0; index < tooltip.length; index++) {
      const element = tooltip[index];
      element.addEventListener("click", (e) => {
         const targetElement = e.target;
         targetElement.classList.add('_show-tooltip');
         window.navigator.clipboard.writeText(targetElement.innerText)
         window.setTimeout(() => {
            targetElement.classList.remove('_show-tooltip');
         }, 1000);
      })
   }
}
